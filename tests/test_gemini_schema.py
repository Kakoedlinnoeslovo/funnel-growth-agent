...
def test_schema_version_is_three_everywhere_and_v2_caches_rerun(settings, tmp_path: Path) -> None:
    from funnel_growth_agent.config import Settings

    assert ANALYSIS_SCHEMA_VERSION == 3
    assert Settings.__dataclass_fields__["analysis_schema_version"].default == 3
    assert settings.analysis_schema_version == 3
    calls = {"n": 0}
    ...
    assert calls["n"] == 2 and cached.schema_version == 3
